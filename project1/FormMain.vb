...
Public Class FormMain
    ...
    Private Sub LoadBlotterCases()
        Try
            Using conn As New MySqlConnection(connectionString)
                ...
                ' We select the main fields for the grid, not the full narrative
                ' Order by newest first
                Dim query As String = "
                SELECT id, incident_date, complainant, respondent, incident_type, status 
                FROM blotter_cases 
            ...
            "
                ...
                Using adapter As New MySqlDataAdapter(query, conn)
                    ...
                    adapter.Fill(dtBlotter)
                    dgvBlotter.DataSource = dtBlotter

                    ' Format the grid
                    If dgvBlotter.Columns.Contains("id") Then dgvBlotter.Columns("id").Visible = False
                    If dgvBlotter.Columns.Contains("incident_date") Then dgvBlotter.Columns("incident_date").HeaderText = "Date of Incident"
                    ...
                    If dgvBlotter.Columns.Contains("incident_type") Then dgvBlotter.Columns("incident_type").HeaderText = "Incident Type"
                    If dgvBlotter.Columns.Contains("status") Then dgvBlotter.Columns("status").HeaderText = "Status"
                End Using
            End Using
        Catch ex As Exception
            MessageBox.Show("Error loading blotter cases: " & ex.Message, "Database Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Private Sub btnSaveCase_Click(sender As Object, e As EventArgs) Handles btnSaveCase.Click
        ...
        ' 1. VALIDATION
        If String.IsNullOrWhiteSpace(txtComplainant.Text) OrElse
           String.IsNullOrWhiteSpace(txtRespondent.Text) OrElse
           String.IsNullOrWhiteSpace(cmbIncidentType.Text) OrElse
           String.IsNullOrWhiteSpace(txtLocation.Text) OrElse
           String.IsNullOrWhiteSpace(cmbStatus.Text) Then

            MessageBox.Show("Please fill in all required fields: Complainant, Respondent, Type, Location, and Status.", "Missing Data", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If

        ' This manual check is much safer than setting .MaxDate
        ...
        ' 2. GET DATA FROM FORM
        Dim complainant As String = txtComplainant.Text.Trim()
        Dim respondent As String = txtRespondent.Text.Trim()
        Dim incidentType As String = cmbIncidentType.Text
        Dim location As String = txtLocation.Text.Trim()
        Dim incidentDate As DateTime = dtpIncidentDate.Value
        Dim status As String = cmbStatus.Text
        Dim narrative As String = txtNarrative.Text.Trim() ' This one can be blank
        ...
        Try
            Using conn As New MySqlConnection(connectionString)
                ...
                (complainant, respondent, incident_type, location, incident_date, status, narrative) 
                VALUES 
                (@Complainant, @Respondent, @IncidentType, @Location, @IncidentDate, @Status, @Narrative)
            "
                Using cmd As New MySqlCommand(query, conn)
            ...
            LoadBlotterCases() ' Refresh the grid
            ClearBlotterForm() ' Call helper to clear the form

        Catch ex As Exception
            MessageBox.Show("Error saving case: " & ex.Message, "Database Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Private Sub btnUpdateCase_Click(sender As Object, e As EventArgs) Handles btnUpdateCase.Click
        ...
        If dgvBlotter.CurrentRow Is Nothing Then
            MessageBox.Show("Please select a case from the grid to update.", "No Case Selected", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        ...
        Try
            ...
            "
                Using cmd As New MySqlCommand(query, conn)
                    ...
                    cmd.Parameters.AddWithValue("@Status", status)
                    cmd.Parameters.AddWithValue("@Narrative", narrative)
                    cmd.Parameters.AddWithValue("@CaseID", selectedCaseId) ' The most important parameter
            ...
            LoadBlotterCases() ' Refresh the grid
            ClearBlotterForm() ' Clear the textboxes

        Catch ex As Exception
            MessageBox.Show("Error updating case: " & ex.Message, "Database Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
